...
The **endpoint**, in API parlance, is the destination URL for your API requests. The endpoint will vary depending on the FormAssembly edition you are interacting with.
...
The key point of OAuth2 is that it allows a **user** of FormAssembly (let's call them Adam), to authorize a **client** (you) to take actions in FormAssembly as if you were Adam. For example, you could write an application, which when authorized by Adam, could download and display a list of all of Adam's forms along with associated metrics, like number of responses, dropout rates, etc.  This data could then be used by you to generate custom reports for Adam.
...
where:
  ...
  + CLIENT_ID is the unique client ID generated by FormAssembly for your client application (See [Registration](#registration) above)
  + RETURN_URL is the URL that your user will be returned to when they complete the authorization step on FormAssembly. This value must be URI-encoded.  

##### 2. Retrieve the Authorization Code

FormAssembly will append the "code" query string parameter to your RETURN_URL when returning the user to the URL. For instance, `redirect_uri=https%3A%2F%2Fwww.yourapp.com` (Note the value is URI-encoded) will result in the user being taken to: `https://www.yourapp.com?code=XXXXXXXXXX` after authorizing FormAssembly.
...
Store the access_token value. You will need it for every API request on the user account. Since the access token is valid for only one user account, you will typically obtain multiple access_token from multiple users, and you must store and manage those tokens accordingly.
...
Returns a list of all forms in the FormAssembly instance. Only accessible if using FormAssembly Enterprise and access_token from an admin-level user.
...
Unique integer value identifying the form within the FormAssembly instance.  Every form has a single unique ID in the form of an integer. Can be used to construct a valid form URL, e.g., http://app.formassembly.com/forms/view/XXXX
